...
[![Firmward](https://github.com/urob/zmk-config/actions/workflows/build-nix.yml/badge.svg)](https://github.com/urob/zmk-config/actions/workflows/build-nix.yml)
...
The repository doubles as the root of the ZMK workspace: a single manifest file declares the entire
...
a nix environment that sets itself up automagically whenever you enter the workspace.
## Highlights

### Toolchain

- [Top-down workspace](#the-config-is-the-workspace) bootstrapped from a single declarative
  manifest
- [Fully locked firmware](#one-manifest-fully-locked) via
...
If you are looking to adapt this config for your own keyboard, [Part III](#part-iii-adapting-this-config)
has the short version.

## Part I: The workspace

### The config is the workspace

ZMK's official `build-user-config` workflow is "config-centric": your `west.yml` declares what goes
into the firmware, and the cloud build takes it from there. The [official local
setup](https://zmk.dev/docs/development/local-toolchain/setup) inverts this: one initializes the
workspace from ZMK's own manifest, and then adds the config and any other modules on top by hand.

This repo keeps [`config/west.yml`](https://github.com/urob/zmk-config/blob/main/config/west.yml) as
the single source of truth for both. It declares ZMK, Zephyr and every module — each [pinned to an
exact revision](#one-manifest-fully-locked) rather than a moving branch — and everything else
follows from it: `just init` bootstraps the workspace from the manifest, a self-installing nix
environment supplies the entire toolchain, and local and cloud builds stay in sync by construction.

What that buys, concretely:

- **One thing to fork.** There are no separate ZMK and ZMK module repos to clone and keep in sync.
  The entire firmware is declared in a single manifest file in the same repo that declares the
  keymap itself.
- **A clean line between tracked and generated.** `zmk/`, `zephyr/` and `modules/` are build
  artifacts, not things you maintain: delete them and `just init` recreates them exactly.
- **Nothing to assemble by hand.** Clone the repo and run `just init` to set up a local workspace
  mirroring the cloud build.
...
├── firmware/          # compiled firmware ends up here build
│
├── modules/           # ─── generated, not tracked in git ───
├── zephyr/
└── zmk/
```
...
- A [scheduled workflow](https://github.com/urob/zmk-config/blob/main/.github/workflows/bump-west.yml)
  does the same weekly and opens a PR, so updates only land once they build green.

As a bonus, the manifest restricts Zephyr's module imports to an allowlist of what ZMK actually
needs, avoiding the download of several GBs of unused Zephyr modules. (If you build for less common
hardware, you may need to extend the allowlist — see the comments in `west.yml`.)

### Building the firmware

**In the cloud.** Push your changes and download the firmware from the repository's `Actions` tab
— no local setup required. (GitHub disables Actions on newly created forks; enable them once from
the fork's `Actions` tab before the first push.) The build runs on the same nix environment as the
local setup, which keeps both in lockstep and avoids maintaining a second toolchain definition. The
stock ZMK workflow is kept around for reference and remains available via manual dispatch.
...
system. Once the prerequisites are in place, the entire setup is:
...
After this initial setup, entering the workspace automatically activates the build environment, `just
build all` compiles every target in `build.yaml`, `just flash <target>` flashes non-UF2 boards, and
`just draw` re-renders the keymap images. Running `just` without arguments lists the complete list of 
available recipes.

See [`docs/build-env.md`](docs/build-env.md) for the prerequisites, the full recipe reference, and
notes on hacking the ZMK source and modules. The environment itself is [continuously
...
keys. Adding a board means writing one such adapter and registering it in `build.yaml`.
...
- the bottom vertical-combo row is symmetric to the top row (subscript `_` aligns with superscript
  `^`; minus `-` aligns with `+`; division `/` aligns with multiplication `*`; logical-or `|`
...
- left-hand side combos for `tab`, `esc`, `cut` (on <kbd>X</kbd> + <kbd>D</kbd>), `copy` and
  `paste` that go well with right-handed mouse usage
...
This config is written for my own keyboards. If you are looking to adopt it, here's the main steps:
...
- [pin-west](https://github.com/urob/pin-west), the manifest-locking tool used to [pin the
  firmware](#one-manifest-fully-locked). It works with any west workspace, not just this one.
